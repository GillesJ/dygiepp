...
"""
Adapted from predict_dygie.py for error inspection and analysis.
...
usage: predict.py [archive-file] [test-file] [predictions-output-file] [score-output-dir] [cuda-device-id]
"""
...
def main():
    if len(argv) > 1: # parse cmd args if specified
        archive_file = argv[1]
        test_file = argv[2]
        output_file = argv[3]
        ...
        archive_file = f"{base_dir}models/sentivent-event-nonerforargs/model.tar.gz"
        test_file = f"{base_dir}data/sentivent/ner_with_subtype_args/test.jsonl"
        output_file = f"{base_dir}tmp/sentivent-nonerforargs-predictions_test.jsonl"
        cuda_device = 0
        score_dir = f"{base_dir}tmp/sentivent-nonerforargs-scores/"
        if score_dir is not None:
            if not path.exists(score_dir):
                os.mkdir(score_dir)
